...
//  HW 4 Q 7
//
//  Created by Joseph Picchi on 5/27/19.
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    {
        // choose the last item in the set
        Sensor nextSensor = s[k-1];
        
        // shift to the right until we get the proper slot
        int loc = k-2;
        while (loc >= 0 && comp(nextSensor, s[loc]))
        {
            s[loc+1] = s[loc];
            loc--;
        }
        // place the sensor into the right slot
        s[loc+1] = nextSensor;
    }
}
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Sensor*> sensorPtrs(nsensors);
